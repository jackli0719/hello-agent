...
describe("releaseOrderAction", () => {
  ...
  it("assigned 订单完成 → 订单 completed，师傅 busy → available", async () => {
    // O20260628003 初始 assigned + T002 busy
    const before = await prisma.master.findUnique({ where: { id: "T002" } });
    expect(before?.status).toBe("busy");

    const r = await releaseOrderAction("O20260628003", "completed");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(r.orderId).toBe("O20260628003");
  ...
  it("in_service 订单完成 → 师傅也释放", async () => {
    // O20260629011 初始 in_service + T001 busy
    const r = await releaseOrderAction("O20260629011", "completed");
    expect(r.ok).toBe(true);
  ...
  it("订单取消 → 师傅也释放", async () => {
    // 先把 O20260628003 改成 assigned + T002 busy（已经是这个状态）
    const r = await releaseOrderAction("O20260628003", "cancelled");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  it("pending 订单没有师傅可释放 → validation 错误", async () => {
    const r = await releaseOrderAction("O20260629001", "completed");
  ...
  it("completed 订单再完成一次 → validation 错误（已释放过）", async () => {
    // O20260626001 初始 completed（且没有 masterId）
    const r = await releaseOrderAction("O20260626001", "completed");
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.category).toBe("validation");
  });
  ...
  it("非法状态参数 → validation 错误", async () => {
    ...
    const r = await releaseOrderAction("O20260628003", "pending" as never);
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.category).toBe("validation");
  });
