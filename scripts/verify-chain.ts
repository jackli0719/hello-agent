...
(async () => {
  ...
  try {
    ...
    const catObj = await prisma.serviceCategory.findUnique({
      where: { id: ids.cat1! },
    });
    const skuObj = await prisma.serviceSku.findUnique({
      where: { id: ids.sku1! },
    });
    ...
    console.log(
      "   候选:",
      r7.candidates.map((c) => c.id).join(","),
      "（应只有 m1 因为 m2 不会空调维修）",
    );

    // 8. 无规则 → 兜底推荐
    const noRules = rules.filter(
      (r) =>
        r.spec.match.skuId !== skuObj!.id &&
        r.spec.match.categoryId !== catObj!.id,
    );
    ...
    console.log(
      "8. 无规则 → rule=null, candidates=[]:",
      r8.rule === null,
      r8.candidates.length === 0,
    );
    ...
    if (ids.o1 && ids.m2) {
      ...
      console.log(
        ...
        !tryAgain.ok,
        "原因:",
        tryAgain.ok === false ? tryAgain.error : "n/a",
      );
    }
    ...
    for (const key of ["o1", "m1", "m2", "r1", "r2", "sku1", "cat1"]) {
      ...
      if (key.startsWith("o")) await prisma.order.deleteMany({ where: { id } });
        ...
        await prisma.serviceSku.deleteMany({ where: { id } });
