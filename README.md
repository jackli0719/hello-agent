...
> **当前版本：Beta MVP（v0.9.2）** —— 三端完整演示闭环已跑通，欢迎试用反馈。
>
...
| 文档                                                | 用途                                                                                           |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 🚀 [docs/DEMO.md](docs/DEMO.md)                     | **5 分钟快速上手** + 4 步演示脚本 + 重置数据                                                   |
| ✅ [docs/BETA_CHECKLIST.md](docs/BETA_CHECKLIST.md) | **17 项 Beta 验收清单**（用户端 × 3 + 后台 × 7 + 师傅端 × 4 + 取消/备注 + 重置数据）           |
| 📝 [docs/FEEDBACK.md](docs/FEEDBACK.md)             | **试用反馈模板**（评分 1-5 + 卡点 + 付费推荐意愿）                                             |
| 🗺️ [docs/ROADMAP.md](docs/ROADMAP.md)               | **下一阶段路线图**（P0 必修 / P1 短期 / P2 中期 / P3 长期）                                    |
| ⚠️ [docs/KNOWN_ISSUES.md](docs/KNOWN_ISSUES.md)     | **当前已知限制**（支付 / 短信 / 地图 / 部署 / 权限 / 财务 / 评价 / 售后 / 商家端 / Demo 重置） |

试用流程：跑 `npm run seed:demo` 重置数据 → 按 [DEMO.md](docs/DEMO.md) 演示 → 按 [BETA_CHECKLIST.md](docs/BETA_CHECKLIST.md) 打勾 → 填 [FEEDBACK.md](docs/FEEDBACK.md) 反馈。
...
详细已知限制（10 项主要限制 + 4 项小限制）见 **[docs/KNOWN_ISSUES.md](docs/KNOWN_ISSUES.md)**。
...
- ❌ 短信验证码 / 通知
- ❌ 地图定位 + 距离计算
- ❌ 真实线上部署（本地 Docker PostgreSQL）
- ❌ 多角色权限矩阵（admin / worker / customer 三角色固定）
- ❌ 财务结算 + 提现流程
- ❌ 评价系统（师傅评分硬编码初始值）
- ❌ 售后 / 工单系统
- ❌ 商家端（单租户，无 SaaS 多租户）
- ❌ Demo 数据可被重置（演示阶段需要，生产环境需保护）

---

## 7. 下一阶段规划

完整路线图（**P0 必修 / P1 短期 / P2 中期 / P3 长期** + 已完成历史）见 **[docs/ROADMAP.md](docs/ROADMAP.md)**。

P0 当前已完成 5 项（v0.9.0-v0.9.1）：

- ✅ 取消订单时 cancelReason 必填（所有状态）
- ✅ 订单金额必须 > 0
- ✅ 师傅 skills 非空
- ✅ 派单规则 requiredSkills 非空
- ✅ 4 个表单 form action legacy bug 修复

下一阶段优先：P1-1 真实登录 → P2-1 支付 → P2-2 短信 → P2-3 地图 → P3-1 线上部署。
